...
mx::string _GetPortType(mx::PortElementPtr port)
{
...
// Workaround for implicit type conversion issues found in many assets, for example OpenPBR Shader Playground.
// Causes problems with the MDL backend: https://github.com/AcademySoftwareFoundation/MaterialX/issues/1038
void _PatchImplicitTypeConversions(mx::DocumentPtr document)
{
  std::unordered_set<mx::ElementPtr> addedNodes;

  for (auto treeIt = document->traverseTree(); treeIt != mx::TreeIterator::end(); ++treeIt)
  {
    mx::ElementPtr elem = treeIt.getElement();

    if (!elem || elem->getActiveSourceUri() != document->getSourceUri() || addedNodes.contains(elem->getParent()))
    {
      continue;
    }
    ...
    mx::string inputType = _GetPortType(input);
    mx::string outputType = _GetPortType(output);
    ...
    std::string conversionND = GB_FMT("ND_convert_{}_{}", inputType, outputType);
    if (!document->getNodeDef(conversionND))
    {
      GB_WARN("invalid connection types between {} ({}) -> {} ({})",
        input->getName(), inputType, output->getName(), outputType);
      continue;
    }

    std::string nodeCategory = "convert";
    std::string nodeName = mx::EMPTY_STRING; // auto-assign
    std::string nodeType = inputType;
    ...
    GB_DEBUG("patched invalid connection types between {} ({}) -> {} ({})",
      input->getName(), inputType, output->getName(), outputType);
  }
}
...
namespace gtl
{
  ...
  mx::DocumentPtr McMtlxDocumentPatcher::patch(const mx::DocumentPtr document)
  {
    ...
    _PatchImplicitTypeConversions(docCopy);
