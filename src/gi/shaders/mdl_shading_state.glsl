...
layout(buffer_reference, std430, buffer_reference_align = 4) buffer BlasPayloadIndices {
  Face data[];
};
layout(buffer_reference, std430, buffer_reference_align = 32) buffer BlasPayloadVertices {
  FVertex data[];
};

void setup_mdl_shading_state(in vec2 hit_bc, out State state)
{
    BlasPayload payload = blas_payloads.payloads[gl_InstanceCustomIndexEXT];
    BlasPayloadIndices indices = BlasPayloadIndices(payload.indexVertexBuffer);
    BlasPayloadVertices vertices = BlasPayloadVertices(payload.indexVertexBuffer);
