    ...
    // ReBAR check
    {
      ...
      for (uint32_t i = 0; i < memoryProperties.memoryTypeCount; i++)
      {
        ...
        if (bool(memoryType.propertyFlags & VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT) && heapSize > largestDeviceLocalHeapSize)
        {
          ...
          isHeapHostAccessible =
            bool(memoryType.propertyFlags & VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT) &&
            bool(memoryType.propertyFlags & VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
        }
      }
